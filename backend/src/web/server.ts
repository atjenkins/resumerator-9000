...
const upload = multer({
  ...
  fileFilter: (_req, file, cb) => {
    ...
    if (allowedTypes.includes(file.mimetype) || allowedExtensions.includes(ext)) {
      ...
      cb(new Error('Invalid file type. Please upload PDF, DOCX, TXT, or MD files.'));
    }
  },
});
...
// Initialize project
app.post('/api/project/init', async (_req: Request, res: Response, next: NextFunction): Promise<void> => {
  try {
...
// List people
app.get('/api/project/people', async (_req: Request, res: Response, next: NextFunction): Promise<void> => {
  try {
    const { project } = getManagers();
...
// List companies
app.get('/api/project/companies', async (_req: Request, res: Response, next: NextFunction): Promise<void> => {
  try {
    const { project } = getManagers();
...
app.get('/api/project/jobs/:company', async (req: Request, res: Response, next: NextFunction): Promise<void> => {
  try {
    const { project } = getManagers();
...
// Get person content
app.get('/api/project/person/:name', async (req: Request, res: Response, next: NextFunction): Promise<void> => {
  try {
    const { project } = getManagers();
...
// Get company content
app.get('/api/project/company/:name', async (req: Request, res: Response, next: NextFunction): Promise<void> => {
  try {
    const { project } = getManagers();
...
// Get job content
app.get('/api/project/job/:company/:job', async (req: Request, res: Response, next: NextFunction): Promise<void> => {
  try {
    const { project } = getManagers();
    const content = project.getJobContent(req.params.company as string, req.params.job as string);
    res.json({ content });
  } catch (error) {
    next(error);
  }
...
// Add person
app.post('/api/project/person', async (req: Request, res: Response, next: NextFunction): Promise<void> => {
  try {
    const { name } = req.body;
    if (!name) {
...
// Add company
app.post('/api/project/company', async (req: Request, res: Response, next: NextFunction): Promise<void> => {
  try {
    const { name } = req.body;
    if (!name) {
      res.status(400).json({ error: 'Name is required' });
...
// Add job
app.post('/api/project/job', async (req: Request, res: Response, next: NextFunction): Promise<void> => {
  try {
...
// List results
app.get('/api/results', async (req: Request, res: Response, next: NextFunction): Promise<void> => {
  try {
...
// Get result content
app.get('/api/results/:filename', async (req: Request, res: Response, next: NextFunction): Promise<void> => {
  try {
    const { results } = getManagers();
  ...
  async (req: Request, res: Response, next: NextFunction): Promise<void> => {
    try {
      ...
      if (mode === 'create') {
        // Create new person
        if (!newPersonName) {
          res.status(400).json({ error: 'New person name is required for create mode' });
          return;
        }
        ...
        if (!personName) {
          res.status(400).json({ error: 'Person name is required for merge mode' });
          return;
        }
        ...
        res.status(400).json({ error: 'Mode must be either "create" or "merge"' });
      }
    } catch (error) {
      next(error);
    }
...
app.post('/api/import/job', async (req: Request, res: Response, next: NextFunction): Promise<void> => {
  try {
    const { mode, company, jobName, jobText } = req.body;
    const { project } = getManagers();

    if (!company || !jobName || !jobText) {
      res.status(400).json({ error: 'Company, job name, and job text are required' });
      return;
    }
    ...
    if (mode === 'create') {
      ...
      const appended = await agent.appendJobDescription(existingContent, jobText);
      ...
      res.status(400).json({ error: 'Mode must be either "create" or "append"' });
    }
  } catch (error) {
    next(error);
  }
...
app.post('/api/import/company', async (req: Request, res: Response, next: NextFunction): Promise<void> => {
  try {
    const { mode, companyName, companyText } = req.body;
    const { project } = getManagers();

    if (!companyName || !companyText) {
      res.status(400).json({ error: 'Company name and company text are required' });
      return;
    }
    ...
    if (mode === 'create') {
      ...
      const appended = await agent.appendCompanyInfo(existingContent, companyText);
...
// Get person file
app.get('/api/files/person/:name', async (req: Request, res: Response, next: NextFunction): Promise<void> => {
  try {
    const { project } = getManagers();
...
// Update person file
app.put('/api/files/person/:name', async (req: Request, res: Response, next: NextFunction): Promise<void> => {
  try {
...
// Get company file
app.get('/api/files/company/:name', async (req: Request, res: Response, next: NextFunction): Promise<void> => {
  try {
    const { project } = getManagers();
    const name = Array.isArray(req.params.name) ? req.params.name[0] : req.params.name;
...
// Update company file
app.put('/api/files/company/:name', async (req: Request, res: Response, next: NextFunction): Promise<void> => {
  try {
    const { content } = req.body;
    if (!content) {
      res.status(400).json({ error: 'Content is required' });
...
// Get job file
app.get('/api/files/job/:company/:job', async (req: Request, res: Response, next: NextFunction): Promise<void> => {
  try {
    const { project } = getManagers();
    const company = Array.isArray(req.params.company) ? req.params.company[0] : req.params.company;
    const job = Array.isArray(req.params.job) ? req.params.job[0] : req.params.job;
...
// Update job file
app.put('/api/files/job/:company/:job', async (req: Request, res: Response, next: NextFunction): Promise<void> => {
  try {
    const { content } = req.body;
    if (!content) {
      res.status(400).json({ error: 'Content is required' });
...
app.get('/api/files/person/:name/resumes', async (req: Request, res: Response, next: NextFunction): Promise<void> => {
  try {
    const { project } = getManagers();
    const name = Array.isArray(req.params.name) ? req.params.name[0] : req.params.name;
...
app.get('/api/files/person/:name/resumes/:resume', async (req: Request, res: Response, next: NextFunction): Promise<void> => {
  try {
    const { project } = getManagers();
    const name = Array.isArray(req.params.name) ? req.params.name[0] : req.params.name;
    const resume = Array.isArray(req.params.resume) ? req.params.resume[0] : req.params.resume;
...
app.put('/api/files/person/:name/resumes/:resume', async (req: Request, res: Response, next: NextFunction): Promise<void> => {
  try {
    const { content } = req.body;
    if (!content) {
      res.status(400).json({ error: 'Content is required' });
        ...
        const savedPath = results.saveResult(
          {
            ...
            person: personName,
            personFile: personName ? `people/${personName}/person.md` : undefined,
            company: companyName,
            companyFile: companyName ? `companies/${companyName}/company.md` : undefined,
            job: jobName,
...
app.post(
  ...
  async (req: Request, res: Response, next: NextFunction): Promise<void> => {
    try {
      ...
      // Save result if requested
      if ((req.body.save === 'true' || req.body.save === true) && personName && companyName && jobName) {
        // Save to person's resumes folder
        response.resumePath = project.saveResume(personName, companyName, jobName, result.markdown);
